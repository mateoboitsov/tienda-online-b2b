...
import { ChevronsUpDown, Apple, Smartphone, Laptop, Tablet, Watch, Headphones, CheckCircle2 } from "lucide-react";
  ...
  return (
    <>
      ...
      <Dialog open={showSuccessModal} onOpenChange={setShowSuccessModal}>
        <DialogContent className="sm:max-w-md bg-white rounded-3xl border-none p-8 text-center overflow-hidden">
          <div className="absolute top-0 left-0 w-full h-2 bg-brand-neon" />
          <div className="flex flex-col items-center gap-6 pt-4">
            <div className="w-20 h-20 bg-brand-neon/10 rounded-full flex items-center justify-center animate-bounce-subtle">
              <CheckCircle2 className="w-12 h-12 text-brand-neon" />
            </div>

            <div className="space-y-2">
              <DialogTitle className="text-3xl font-bold text-brand-black tracking-tight">
                ¡GENIAL!
              </DialogTitle>
              <DialogDescription className="text-lg text-brand-black/70 font-medium">
                Hemos recibido tu solicitud correctamente.
              </DialogDescription>
            </div>

            <p className="text-brand-black/60 max-w-[280px] mx-auto">
              Nuestro equipo te contactará pronto con las mejores ofertas personalizadas para tu negocio.
            </p>

            <Button
              onClick={() => setShowSuccessModal(false)}
              className="w-full bg-brand-black text-white hover:bg-brand-black/90 h-12 rounded-xl font-bold text-lg transition-all duration-300"
            >
              ENTENDIDO
            </Button>
          </div>
